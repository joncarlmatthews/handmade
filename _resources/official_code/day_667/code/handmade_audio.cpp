...
#define AudioStateOutputChannelCount 2
        ...
        )
    {
        ...
        {
            s16 *Samples = GetSoundSamples(Assets, PlayingSound->ID);
            if(Samples)
            {
                hha_asset *LoadedSound = GetSoundInfo(Assets, PlayingSound->ID);
                ...
                u32 SampleCount = (LoadedSound->DataSize / sizeof(s16));
                u32 ChunksToMix = TotalChunksToMix;
                r32 RealChunksRemainingInSound =
                    (SampleCount - RoundReal32ToInt32(PlayingSound->SamplesPlayed)) / dSampleChunk;
                    ...
                    ++LoopIndex)
                {
                    ...
                    __m128 SampleValueF = _mm_setr_ps(Samples[((int32 *)&SampleIndex)[0]],
                                                      Samples[((int32 *)&SampleIndex)[1]],
                                                      Samples[((int32 *)&SampleIndex)[2]],
                                                      Samples[((int32 *)&SampleIndex)[3]]);
                    __m128 SampleValueC = _mm_setr_ps(Samples[((int32 *)&SampleIndex)[0] + 1],
                                                      Samples[((int32 *)&SampleIndex)[1] + 1],
                                                      Samples[((int32 *)&SampleIndex)[2] + 1],
                                                      Samples[((int32 *)&SampleIndex)[3] + 1]);
                    
                    __m128 SampleValue = _mm_add_ps(_mm_mul_ps(_mm_sub_ps(One, Frac), SampleValueF),
                                                    _mm_mul_ps(Frac, SampleValueC));
#else                    
                    __m128 SampleValue = _mm_setr_ps(Samples[RoundReal32ToInt32(SamplePosition + 0.0f*dSample)],
                                                     Samples[RoundReal32ToInt32(SamplePosition + 1.0f*dSample)],
                                                     Samples[RoundReal32ToInt32(SamplePosition + 2.0f*dSample)],
                                                     Samples[RoundReal32ToInt32(SamplePosition + 3.0f*dSample)]);
#endif
                ...
                if(ChunksToMix == ChunksRemainingInSound)
                {
                    if(IsValid(NextSoundInChain))
                    {
                        ...
                        Assert(PlayingSound->SamplesPlayed >= SampleCount);
                        PlayingSound->SamplesPlayed -= (f32)SampleCount;
